      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {reisen.map((reise) => (
          <div key={reise.id} className="bg-bgColor-100 rounded-lg overflow-hidden shadow-lg">
            ...
            <div className="p-4">
              <h2 className="text-lg font-bold text-primaryColor-100 mb-2">{reise.title}</h2>
              <p className="text-textColor-100 mb-2">{reise.description}</p>
              <Link to={reise.link}>
                <button className="bg-textColor-100 text-bgColor-100 px-4 py-2 rounded font-semibold hover:bg-primaryColor-200">
                  Mehr erfahren
                </button>
              </Link>
            </div>
